...
var server = http.createServer(function (request, response) {
  ...
  if(path === '/sign_in'&& method === "POST"){
    ...
    request.on('end',()=>{
      const string = Buffer.concat(array).toString()//把经过utf-8编码的数据转回我们看得懂的数据
  ...
  //上面是动态服务器，下面是静态
    ...
    const index = filePath.lastIndexOf('.')//寻找路径里的最后一个点
    const FilenameExtension = filePath.substring(index)//取圆点以及之后的所有内容
    const fileTypes = {
      '.html': 'text/html',
      '.css': 'text/css',
      'js': 'text/javascript',
    ...
    response.setHeader('Content-Type', `${fileTypes[FilenameExtension] || 'text/html'};charset=utf-8`)
    let content
    try {
